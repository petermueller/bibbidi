defmodule Bibbidi.CDDL.Generator do
  ...
  defp generate_events_module(igniter, mod, events, all_rules, type_refs) do
    ...
    event_functions =
      ...
      |> Enum.map(fn {method, params_ref} ->
        ...
        examples = Examples.for_name("Bibbidi.Events.#{camel}.#{fun_name}")

        """
          @doc \"\"\"
          Event: `#{method}`

          Params type: `#{params_ref || "none"}`#{examples}
          \"\"\"
  ...
  defp generate_event_struct_module(igniter, snake_mod, mod, method, params_ref, fields, type_refs) do
    ...
    examples = Examples.for_name("Bibbidi.Events.#{camel_mod}.#{struct_name}")
  ...
  defp build_command_moduledoc(method, fields, type_refs, name) do
    ...
    examples = Examples.for_name(name)

    "  Command struct for `#{method}`.\n\n  [WebDriver BiDi Spec](#{anchor})#{field_docs}#{examples}"
  end
  ...
  defp generate_facade_function(camel_mod, command_name, method, params_ref, all_rules, type_refs) do
    ...
    examples = Examples.for_name("Bibbidi.Commands.#{camel_mod}.#{fun_name}")

    # Build @doc with opts description and (optionally) examples
    doc =
      cond do
        optional != [] ->
          """
            ...
            \#{Zoi.describe(#{command_name}.opts_schema())}#{examples}
            \"\"\"
          """
        ...
        examples != "" ->
          """
            @doc \"\"\"
            Executes the `#{method}` command.#{examples}
            \"\"\"
          """
