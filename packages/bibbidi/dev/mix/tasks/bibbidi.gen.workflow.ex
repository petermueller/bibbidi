defmodule Mix.Tasks.Bibbidi.Gen.Workflow do
  ...
  - `lib/<app>/op.ex` — pipeline builder
  - `lib/<app>/operation.ex` — execution record struct
  - `lib/<app>/runner.ex` — sequential runner
  - `test/support/mock_transport.ex` — `Bibbidi.Transport` mock for unit-testing
  - `test/<app>/runner_test.exs` — test scaffold using the local mock
  ...
  engine like Runic or Reactor, consider writing a thin adapter instead.
  """
  ...
  def igniter(igniter) do
    ...
    |> generate_mock_transport(prefix)
    |> generate_runner_test(prefix)
  end

  defp generate_mock_transport(igniter, prefix) do
    module = Module.concat([prefix, MockTransport])

    content = ~s'''
    defmodule #{inspect(module)} do
      @moduledoc """
      Mock `Bibbidi.Transport` for unit-testing the workflow runner without
      a real WebSocket. Lives in `test/support/` so it's only loaded in the
      test environment (see `elixirc_paths(:test)` in mix.exs).

      Generated by `mix bibbidi.gen.workflow`. Yours to modify.
      """
    ...
    Igniter.create_new_file(igniter, "test/support/mock_transport.ex", content)
  end

  defp generate_op(igniter, prefix) do
    module = Module.concat([prefix, Op])
    ...
    defmodule #{inspect(module)} do
      ...
      Each command struct exposes Zoi schemas via `schema/0`, `opts_schema/0`,
      and `result_schema/0` for runtime validation.

      ## Usage

          alias #{inspect(prefix)}.Op
          ...
          {:ok, results, operation} = #{inspect(prefix)}.Runner.execute(conn, op)
      """
  ...
  defp generate_operation(igniter, prefix) do
    module = Module.concat([prefix, Operation])
  ...
  defp generate_runner(igniter, prefix) do
    module = Module.concat([prefix, Runner])
    op_module = Module.concat([prefix, Op])
    operation_module = Module.concat([prefix, Operation])
  ...
  defp generate_runner_test(igniter, prefix) do
    module = Module.concat([prefix, RunnerTest])
    op_module = Module.concat([prefix, Op])
    runner_module = Module.concat([prefix, Runner])
    mock_module = Module.concat([prefix, MockTransport])
