...
- Event subscribers receive parsed event structs directly (e.g. `%Bibbidi.Events.BrowsingContext.Load{}`). Use `import Bibbidi.Events.Guards` for category guards, or override the message shape per-subscribe via `Bibbidi.Connection.subscribe/4`'s `:wrap` option.
...
6. (Optional) Drop usage examples into `priv/examples/{module,function}/<FullyQualifiedElixirName>.md`. `mix bibbidi.gen` splices their contents into the generated `@moduledoc` / `@doc` under a `## Examples` heading. See `priv/examples/README.md` for the naming convention and the four supported emission points (command struct, facade builder, event struct, event helper).
...
### Regenerating after CDDL or codegen changes

When `dev/cddl/{parser,generator,utils}.ex` or any `priv/cddl/*.cddl` file changes,
run the regen-codegen skill (`.claude/skills/regen-codegen.md`) — it deletes every
`Generated by mix bibbidi.gen` file under `lib/`, reruns `mix bibbidi.gen --yes`,
and verifies the diff is reproducible. Always run this before committing codegen
changes; the codegen overwrites at paths it computes, and stale files at
no-longer-canonical paths can cause module-redefinition warnings on compile.
